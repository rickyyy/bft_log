...
public class QueryServer extends DefaultRecoverable {
	...
	public int mapToNode (int random, int MAX_SIZE){
		int sanitize = random % MAX_SIZE;
		//ArrayList<Host> list = config.aliveServerList(); TODO add the selection of execution node only among "alive" nodes.
		int totalNumberNodes = this.config.n;
		double executionNode = (sanitize/(double)MAX_SIZE)*totalNumberNodes;
		System.out.print("(" + sanitize + "/" + MAX_SIZE + ")* " + totalNumberNodes + "= " + executionNode);
